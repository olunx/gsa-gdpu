...
public class NewsEduCompo extends Composite {

	private Table listTable;
	private static Listener mySetDataListener = null;
	private Button updateBut;
	// 保存数据的文件
	String fileName = "newsedu.xml";
	 ...
	 * @param style
	 */
	public NewsEduCompo(Composite parent, int style) {
		super(parent, style);
		...
		updateBut.setImage(SWTResourceManager.getImage(NewsEduCompo.class, "/cn/imgdpu/ico/refresh.gif"));
		...
		warnLab.setImage(SWTResourceManager.getImage(NewsEduCompo.class, "/cn/imgdpu/ico/warning_obj.gif"));
		...
		warnLab.setText("所有数据均来源于[Google资讯-教育频道]！小贴士：双击选中项浏览详细内容！O(∩_∩)O");

		CLabel label;
		label = new CLabel(this, SWT.NONE);
		label.setImage(SWTResourceManager.getImage(NewsEduCompo.class, "/cn/imgdpu/ico/template_obj.gif"));
	...
	void getRssData() {
		...
		Thread getData = new Thread() {
			public void run() {
				...
				// 定义获取rss的来源地址
				String domain = "www.google.cn";
				String target = "/news?pz=1&ned=ccn&hl=zh-CN&topic=ed&output=rss";
