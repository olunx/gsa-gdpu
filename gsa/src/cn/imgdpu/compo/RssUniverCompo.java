...
public class RssUniverCompo extends Composite {

	private Table listTable;
	private static Listener mySetDataListener = null;
	private Button updateBut;
	// 保存数据的文件
	String fileName = "rssuniver.xml";
	 ...
	 * @param style
	 */
	public RssUniverCompo(Composite parent, int style) {
		super(parent, style);
		...
		final TableColumn link = new TableColumn(listTable, SWT.NONE);
		link.setWidth(288);
		link.setText("链接");

		final TableColumn description = new TableColumn(listTable, SWT.NONE);
		description.setWidth(100);
		description.setText("分类");
		...
		updateBut.setImage(SWTResourceManager.getImage(RssUniverCompo.class, "/cn/imgdpu/ico/refresh.gif"));
		...
		warnLab.setImage(SWTResourceManager.getImage(RssUniverCompo.class, "/cn/imgdpu/ico/warning_obj.gif"));
		final FormData fd_warnLab = new FormData();
		fd_warnLab.right = new FormAttachment(updateBut, -5, SWT.LEFT);
		...
		warnLab.setText("所有网站数据均来源于人工收集！小贴士：双击选中项浏览详细内容！O(∩_∩)O");

		CLabel label;
		label = new CLabel(this, SWT.NONE);
		label.setImage(SWTResourceManager.getImage(RssUniverCompo.class, "/cn/imgdpu/ico/template_obj.gif"));
	...
	void getRssData() {
		...
		cn.imgdpu.GSAGUI.setStatusAsyn("正在使劲的读取数据中...~(>_<)~");

		Thread getData = new Thread() {
			public void run() {

				// 定义获取rss的来源地址
				String domain = "www.olunx.com";
				String target = "/gsa/rss.php";
				...
				cn.imgdpu.GSAGUI.setStatusAsyn("更新数据成功了！O(∩_∩)O");
			}
		};
	...
	void setTableText(final ArrayList<String> files) {
		class setDataListener implements Listener {
			...
			int index;
			String title, link, date, desctiption;
			...
			public void handleEvent(Event e) {
				...
				desctiption = files.get(index + 2);
				date = files.get(index + 3);

				// 设置表中数据
				item.setText(new String[] { String.valueOf(e.index), title, link, desctiption, date });
				...
				if (listTable.getItems().length > 0) {
					...
					listTable.addListener(SWT.DefaultSelection, new Listener() {
						...
						public void handleEvent(Event e) {
							...
							String url = selectItem[0].getText(2);
					...
					copyUrl.addListener(SWT.Selection, new Listener() {
						...
						public void handleEvent(Event e) {
							...
							for (int i = 0; i < selectItem.length; i++) {
								copys.append(selectItem[i].getText(2) + "\n");
							}
