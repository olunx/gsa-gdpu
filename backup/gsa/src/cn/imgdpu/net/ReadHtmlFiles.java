		...
		try {
			...
			for (int i = 0; i < nodes.length; i++) {
				...
				if(href.endsWith("/")) {
					...
					if(!href.startsWith("http") && !textLink.getLinkText().trim().equals("Name") && !textLink.getLinkText().trim().equals("Last modified") && !textLink.getLinkText().trim().equals("Size") && !textLink.getLinkText().trim().equals("Description")) {
