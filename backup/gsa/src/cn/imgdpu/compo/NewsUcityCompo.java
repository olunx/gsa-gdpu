...
public class NewsUcityCompo extends Composite {

	private Table listTable;
	private static Listener mySetDataListener = null;
	private Button updateBut;
	// 保存数据的文件
	String fileName = "newsucity.xml";
	 ...
	 * @param style
	 */
	public NewsUcityCompo(Composite parent, int style) {
		super(parent, style);
		...
		updateBut.setImage(SWTResourceManager.getImage(NewsUcityCompo.class, "/cn/imgdpu/ico/refresh.gif"));
		...
		warnLab.setImage(SWTResourceManager.getImage(NewsUcityCompo.class, "/cn/imgdpu/ico/warning_obj.gif"));
		...
		warnLab.setText("所有数据均来源于[广州大学城论坛]！小贴士：双击选中项浏览详细内容！O(∩_∩)O");

		CLabel label;
		label = new CLabel(this, SWT.NONE);
		label.setImage(SWTResourceManager.getImage(NewsUcityCompo.class, "/cn/imgdpu/ico/template_obj.gif"));
	...
	void getRssData() {
		...
		Thread getData = new Thread() {
			public void run() {
				...
				// 开始获取数据，并保存到本地
				ReadRssNews readNews = new ReadRssNews();
				readNews.encode = "GBK";
				readNews.getData(domain, target, fileName);
